...
- **Turso**: チェック結果とリソーススナップショットを保持する libSQL データベース。ORM は Drizzle を使用。
  - `snapshots`: 毎分 1 行、全サービス分のチェック結果とリソース値を JSON で保持する生データ。90 日で削除
  - `rollups_30m`: 30 分バケット × サービスの集計 (up/down カウント、リソース合計値)。health-checker が生成し、無期限保持。web の稼働率と 7d/30d チャートはこのテーブルだけを読む
  - `service_meta`: サービスの表示名・カテゴリ
...
- チェック方式は HTTP / TCP / ping / docker の 4 種類。各サービスの方式は `config.ts` で定義
- リソース統計 (CPU / メモリ / ディスク / ネットワーク) は Proxmox API (`status/current`) から取得
- 書き込みごとに、その snapshot が属する 30 分バケットを `rollups_30m` へ再集計 (冪等 upsert)。起動時には未集計期間をバックフィルする
- 90 日より古い `snapshots` レコードを 24 時間ごとに削除 (`rollups_30m` は削除しない)
...
- トップページ: サービスをカテゴリ (public / internal) ごとにグループ表示。各サービスに状態インジケータ・CPU/メモリ使用率・稼働率バー・リソース使用量チャートを縦に並べて表示し、縦スクロールのみで全サービスを閲覧できる（ページ遷移なし）。30 秒ごとに自動更新する
- 直近のチェックが 3 分以上前の場合はデータが古い旨の警告を表示する
- 時間レンジ (24h / 7d / 30d) の切替はページ全体で共有し、稼働率バーとリソース使用量チャートの両方が選択中のレンジを表示する。切替時はページ遷移せず `/api/history` の 1 リクエストで両方を再取得する
- 稼働率とチャートのデータソース: 稼働率 (全レンジ) と 7d/30d チャートは `rollups_30m`、24h チャートは 1 分解像度の `snapshots` を読む
...
- health-checker の `compose.yaml` は `/var/run/docker.sock` を `:ro` マウントして `user: "0:0"` で実行する
- `:ro` はソケットファイルの書き込みのみ制限し、Docker API 経由のあらゆる操作（コンテナ生成・exec・ホスト FS bind-mount など）は制限されない — つまり `docker.sock` アクセスは arona 上のホスト root 権限と等価。コード上は現在 GET リクエストのみ発行している
- 監視対象の 3 コンテナ（`fun-council-bot-1`, `misskey-mixi2-link-bridge-1`, `swarm-gcal-sync-sync-1`）は Docker Compose のデフォルト命名規約（`{project}-{service}-{index}`）に従う
- compose.yaml をリネーム・再構成・複数化するとコンテナ名が変わるので `config.ts` の `container` フィールドも併せて更新する。更新しないとチェックが "no such container" エラーで "down" を報告する
